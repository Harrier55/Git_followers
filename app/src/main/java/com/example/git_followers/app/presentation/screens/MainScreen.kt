...
fun MainScreen(viewModel: MainScreenViewModel = koinViewModel()) {
    ...
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        ...
        userEntity?.let { userEntity ->
            ItemSearchedUser(userEntity)
        }
    }
}

@Composable
fun ItemSearchedUser(userEntity: UserEntity) {
    Column(horizontalAlignment = Alignment.CenterHorizontally) {
        Box()
        {
        ...
        Spacer(Modifier.size(8.dp))
        Text(text = "Подписчиков ${userEntity.userNumFollowers} ", style = MaterialTheme.typography.bodyLarge)
        Spacer(Modifier.size(8.dp))
        Button(onClick = { /*TODO*/ }) {
            Text(text = "Подробнее")
        }
